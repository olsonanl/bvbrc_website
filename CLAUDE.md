...
|---|---|---|
| **Bare token**, no scheme (~287 sites) | p3_api, p3_user | `Authorization: (window.App.authorizationToken \|\| '')` |
| `OAuth <token>` | Workspace service, app service | `WorkspaceManager.js`, `UploadManager.js` |
| `Oauth <token>` (lowercase 'a') | App service stdout/stderr URLs | `JobManager.js` |

**p3_api rejects a prefixed token.** `p3_api/middleware/auth.js` passes the header value verbatim into `validateToken.js`, which does `token.split('|')` and reconstructs the RSA-signed base string from the parts. An `OAuth ` prefix makes the first part parse as key `"OAuth un"`, so the reconstructed string no longer matches what was signed and verification fails.

So the bare convention is **required** for p3_api and `OAuth ` is **required** for workspace/app service. When adding an authenticated request, copy the convention used by other calls to that same service — do not "normalize" them. The Perl side mirrors this (`Shock.pm`, `Awe.pm`, `AppScript.pm` all hardcode `OAuth`).
